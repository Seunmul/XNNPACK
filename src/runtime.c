...
enum xnn_status xnn_create_weights_cache_with_size(
  ...
  cache_provider->is_finalized =
      ...
      //! OFFSET TO ADDR FUNCTION POINTER
...
static enum xnn_status initialize_workspace_values(
  ...
  for (size_t i = 0; i < runtime->num_values; i++) {
    ...
    if (value->allocation_type == xnn_allocation_type_workspace) {
      ...
      value->data = (void*)((uintptr_t)runtime->workspace->data +
      ...
      if (value->datatype == xnn_datatype_qdint8 ||
        ...
        value->quantization.dynamic_params =
            (void*)((uintptr_t)runtime->workspace->data +
                    mem_alloc_tracker->usage[i].alloc_offset +
  ...
  for (size_t i = 0; i < runtime->num_ops; i++) {
    ...
    opdata->workspace =
        (void*)((uintptr_t)runtime->workspace->data + usage->alloc_offset);
  }
...
static bool input_memory_can_be_reused(const xnn_runtime_t runtime,
  ...
  const bool output_memory_fits =
      xnn_runtime_tensor_get_size(input) == xnn_runtime_tensor_get_size(output);
...
static enum xnn_status create_runtime_impl(
  ...
  if (runtime->profiling) {
    for (size_t i = 0; i < subgraph->num_nodes; i++) {
      runtime->opdata[i].end_ts = xnn_allocate_zero_memory(
          sizeof(xnn_timestamp) * XNN_MAX_OPERATOR_OBJECTS);
    }
  }

  runtime->values = xnn_allocate_zero_memory(sizeof(struct xnn_runtime_value) *
                                             subgraph->num_values);
  ...
  for (uint32_t i = 0; i < runtime->num_values; i++) {
    ...
    if (value->flags & XNN_VALUE_FLAG_FP16_COMPATIBLE &&
...
enum xnn_status xnn_create_runtime_with_scheduler(
  ...
  return create_runtime_impl(subgraph, weights_cache, /*workspace=*/NULL,
                             /*threadpool=*/NULL, scheduler, flags,
                             runtime_out);
}
...
enum xnn_status xnn_reshape_runtime(xnn_runtime_t runtime) {
  ...
  if (use_slinky) {
    ...
    if (!runtime->slinky_pipeline ||
        (runtime->flags & XNN_FLAG_SLINKY_STATIC_BOUNDS) != 0) {
...
enum xnn_status xnn_setup_runtime(
    ...
    const struct xnn_external_value* external_values) {
  enum xnn_status status =
      set_external_values(runtime, num_external_values, external_values);
  if (status != xnn_status_success) {
    return status;
  }
...
//   printf("[END INVOKE at XNNPACK runtime.c]\n\n");
  return xnn_status_success;
}
